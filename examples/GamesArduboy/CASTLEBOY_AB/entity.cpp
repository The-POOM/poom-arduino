...
// Plain char is signed on AVR but unsigned on the ESP32-C5 toolchain.  This
// function deliberately returns -1, so retain the original signed byte.
int8_t getMovingPlatformDirection(Entity& entity)
{
...
void updateBird(Entity& entity)
{
  ...
  if (Poom.everyFrames(ENTITY_BIRD_WALK_INTERVAL))
  {
    if (entity.state & FLAG_MISC1)
    {
...
void updateBossKnight(Entity& entity)
{
  ...
  if (Poom.everyFrames(BOSS_KNIGHT_WALK_INTERVAL))
  {
    if (entity.state & FLAG_MISC1)
    {
...
void updateBossHarpy(Entity& entity)
{
  ...
  {
    // flying
    if (Poom.everyFrames(BOSS_HARPY_WALK_INTERVAL))
    {
      if (entity.state & FLAG_MISC1)
      {
        entity.frame = entity.frame == 6 ? 5 : 6;
...
void updateProjectile(Entity& entity)
{
  ...
  if (entity.type == ENTITY_BONE)
  {
    ...
    if (entity.counter < 8 && Poom.everyFrames(10))
    {
      ++entity.counter;
    }
  }
      ...
      else if (entity.state & FLAG_ALIVE)
      {
        switch (entity.type)
        {
          ...
          case ENTITY_PICKUP_KNIFE:
            ...
            if (entity.type != ENTITY_PICKUP_KNIFE && Poom.everyFrames(12))
            {
              ++entity.frame %= 2;
            }
            break;
                            ...
                            x, y, width, height))
      {
        ...
        if (damage)
        {
          if (entity.hp <= value)
          {
            ...
            entity.hp = 0;
            entity.counter = 0;
            Poom.audio().tone(NoteCs3, 30);
          }
          else
          {
            entity.hp -= value;
            Poom.audio().tone(NoteCs3, 15);
          }
        }
      }
    }
                            ...
                            x, y, width, height))
      {
        switch (entity.type)
        {
          ...
          case ENTITY_PICKUP_COIN:
            ...
            Poom.audio().play(SoundPickupCoin);
            break;
          case ENTITY_PICKUP_KNIFE:
...
void Entities::draw()
{
  for (uint8_t i = 0; i < ENTITY_MAX; i++)
  {
    ...
    if (entity.state & FLAG_PRESENT)
    {
      if (entity.state & FLAG_ALIVE || entity.state & MASK_HURT)
      {
        Poom.drawPageAssetInterleavedMasked(entity.pos.x - data[entity.type].spriteOriginX - Game::cameraX, entity.pos.y - data[entity.type].spriteOriginY, data[entity.type].sprite, entity.frame);
#ifdef DEBUG_HITBOX
        Poom.fillRect(entity.pos.x - data[entity.type].hitbox.x - Game::cameraX, entity.pos.y - data[entity.type].hitbox.y, data[entity.type].hitbox.width, data[entity.type].hitbox.height);
#endif
      }
      else
      {
        Poom.drawPageAssetInterleavedMasked(entity.pos.x - DIE_ANIM_ORIGIN_X - Game::cameraX, entity.pos.y - DIE_ANIM_ORIGIN_Y, fx_destroy_plus_mask, entity.frame);
      }
    }
  }
}
