...
{
  Poom.fillRect(x, y, 4 * max, 7, false);
  for (uint8_t i = 0; i < max; i++)
  {
    Poom.drawPageAsset(x + i * 4, y, i < value ? ui_hp_full : ui_hp_empty, 0);
  }
}
...
void Game::play()
{
  ...
  if (Map::boss != NULL)
  {
    Poom.audio().play(SoundBossStart);
    pauseCounter = 120;
  }
}
...
void Game::loop()
{
  // debug
#ifdef DEBUG_CHEAT
//  if (Poom.anyPressed(PoomButtonA) && Poom.anyPressed(PoomButtonB) && Poom.justPressed(PoomButtonDown))
//  {
//    play();
//    return;
//  }

  if (Poom.anyPressed(PoomButtonA) && Poom.anyPressed(PoomButtonB) && Poom.justPressed(PoomButtonUp))
  {
    if (Map::boss != NULL)
    {
...
//  if (Poom.anyPressed(PoomButtonA) && Poom.anyPressed(PoomButtonB) && Poom.justPressed(PoomButtonLeft))
//  {
//    finished = false;
//    mainState = STATE_PLAY;
//    Entities::init();
...
//  }
  ...
  if (paused)
  {
    Poom.overwritePageAsset((PoomScreenWidth / 2) - pgm_read_byte(text_paused) / 2,
                            PoomScreenHeight / 2, text_paused, 0);
    if (Poom.justPressed(PoomButtonB) || Poom.justPressed(PoomButtonA))
    {
      paused = false;
    }
    return;
  ...
  if (Poom.anyPressed(PoomButtonDown) && Poom.justPressed(PoomButtonA))
  {
    paused = true;
    return;
  }
  ...
  {
    // The original levels share this fixed parallax ratio.
    int16_t backgroundOffset = cameraX / 28;
    Poom.overwritePageAsset(16 - backgroundOffset, 4, background_mountain, 0);
  }
  ...
  // ui: knife count
  Poom.fillRect(54, 0, 13, 7, false);
  Poom.drawPageAsset(55, 0, ui_knife_count, 0);
