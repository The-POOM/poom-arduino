...
{  
  ...
  if (knockbackCounter == 0 && attackCounter == 0)
  {
    if (Poom.justPressed(PoomButtonB))
    {
      ...
      Poom.audio().tone(NoteGs4, 10);
    }
    else if (Poom.justPressed(PoomButtonUp))
    {
      if (knifeCount > 0)
      {
        --knifeCount;
        knifeAttack = true;
        attackCounter = PLAYER_ATTACK_TOTAL_DURATION;
        Poom.audio().tone(NoteGs5, 10);
      }
      else
      {
        Poom.audio().tone(NoteG2, 5);
      }
    }
  }

  // jump
  if (knockbackCounter == 0 && !ducking && attackCounter == 0 && grounded && Poom.justPressed(PoomButtonA))
  {
    // start jumping
    grounded = false;
    jumping = true;
  ...
  if (knockbackCounter == 0 && attackCounter == 0)
  {
    if (!ducking)
    {
      ducking = grounded && Poom.anyPressed(PoomButtonDown);
    }
    else if (!Poom.anyPressed(PoomButtonDown))
    {
  ...
  if (knockbackCounter == 0)
  {
    if (attackCounter == 0 && Poom.anyPressed(PoomButtonLeft))
    {
      velocityX = -1;
      flipped = true;
    }
    else if (attackCounter == 0 && Poom.anyPressed(PoomButtonRight))
    {
      velocityX = 1;
      flipped = false;
    }
  ...
  if (velocityX != 0 &&
      (
        // normal speed
        (knockbackCounter == 0 && Poom.everyFrames(ducking ? PLAYER_SPEED_DUCK : PLAYER_SPEED_NORMAL)) ||
        // knockback speed
        (knockbackCounter > 0 && Poom.everyFrames(knockbackCounter < PLAYER_KNOCKBACK_FAST ? PLAYER_SPEED_KNOCKBACK_NORMAL : PLAYER_SPEED_KNOCKBACK_FAST))
      )
     )
  {
...
#ifdef DEBUG_HITBOX
        Poom.fillRect(pos.x + (flipped ? -24 : 0) - Game::cameraX, pos.y - (ducking ? 4 : 12), 24, 2);
#endif
      }
    }
  }
...
void Player::draw()
{
  ...
  else
  {
    if (attackCounter == 0)
    {
      ...
      {
        if (Poom.everyFrames(WALK_FRAME_RATE))
        {
          walkFrame = !walkFrame;
        }
  ...
  if (Poom.everyFrames(4) && knockbackCounter == 0 && invincibleCounter > 0)
  {
    visible = !visible;
  }
  ...
  if (visible)
  {
    Poom.drawPageAssetInterleavedMasked(pos.x - SPRITE_ORIGIN_X - Game::cameraX, pos.y - SPRITE_ORIGIN_Y, player_plus_mask, frame + (flipped ? FRAME_FLIPPED_OFFSET : 0));

    if (attackCounter != 0 && !knifeAttack && attackCounter < PLAYER_ATTACK_CHARGE)
    {
      Poom.drawPageAssetInterleavedMasked(pos.x + (flipped ? -24 : 8) - Game::cameraX , pos.y - (ducking ? 4 : 12), flipped ? player_attack_left_plus_mask : player_attack_right_plus_mask, 0);
    }
  }

  if (knife)
  {
    Poom.drawPageAssetInterleavedMasked(knifePosition.x - Game::cameraX, knifePosition.y, entity_knife_plus_mask, flipped);
  }

#ifdef DEBUG_HITBOX
  Box hitbox = ducking ? duckHitbox : normalHitbox;
  Poom.fillRect(pos.x - hitbox.x - Game::cameraX, pos.y - hitbox.y, hitbox.width, hitbox.height);
#endif
