...
void Game::Draw(int16_t cameraX, int16_t cameraY, uint8_t cameraAngle)
{
	switch(state)
	{
		case State::Menu:
		...
		case State::InGame:
		{
			...
			Renderer::camera.angle = cameraAngle;
...
void Game::Tick()
{
	...
	switch(state)
	{
		...
		case State::GameOver:
			...
			return;
		case State::FadeOut:
			// Fade timing is advanced while the transition is drawn.
			return;
	}
}
