...
void spikesCreate(vec2 pos, byte l)
{
  for (byte i = 0; i < MAX_PER_TYPE; ++i)
  {
    if (!bitRead(spikes[i].characteristics, 2))
    {
      ...
      {
        spikes[i].characteristics = 0b00000111;
      ...
      {
        spikes[i].characteristics = 0b00000101;
        spikes[i].pos.width = len;
        spikes[i].pos.height = 8;
        spikes[i].pos.y += 8;
      }
      ...
      {
        spikes[i].characteristics = 0b00000100;
      ...
      {
        spikes[i].characteristics = 0b00000110;
        spikes[i].pos.width = 8;
        spikes[i].pos.height = len;
        spikes[i].pos.x += 8;
      }
...
void enemiesUpdate()
{
  ...
  if (key.active)
  {
    ...
    Poom.overwritePageAsset(commonx, commony, elements, 4);
  }
  ...
  for (byte i = MAX_PER_TYPE-1; i < MAX_PER_TYPE; --i)
  {
    if (bitRead(spikes[i].characteristics, 2)) // spike active
    {
      ...
      Poom.overwritePageAsset(commonx, commony, sprSpikes,  spikes[i].characteristics & 0b00000011);
      if (!bitRead(spikes[i].characteristics, 0)) {
        for (int l = 8; l < spikes[i].pos.height; l += 8)
          Poom.overwritePageAsset(commonx, commony + l, sprSpikes,  spikes[i].characteristics & 0b00000011);
      }
      else {
        for (int l = 8; l < spikes[i].pos.width; l += 8)
          Poom.overwritePageAsset(commonx + l, commony, sprSpikes,  spikes[i].characteristics & 0b00000011);
      }
    }
  }

  if (Poom.everyFrames(4)) fanFrame = (fanFrame + 1) % 3;
  for (byte i = 0; i < MAX_PER_TYPE; ++i)
  {
    // Fans
    if (fans[i].active)
    {
      ...
      if (Poom.everyFrames(2))
        for (byte a = 0; a < MAX_FAN_PARTICLES; ++a)
        {
          ...
          {
            case FAN_UP:
            Poom.erasePageAsset(fans[i].pos.x + fans[i].particles[a].x - cam.pos.x, fans[i].pos.y - fans[i].particles[a].y - cam.pos.y, particle , 0);
            break;
            case FAN_RIGHT:
            Poom.erasePageAsset(fans[i].pos.x + 16 + fans[i].particles[a].y - cam.pos.x, fans[i].pos.y + 16 - fans[i].particles[a].x - cam.pos.y, particle , 0);
            break;
            default:
            Poom.erasePageAsset(fans[i].pos.x - fans[i].particles[a].y - cam.pos.x, fans[i].pos.y + 16 - fans[i].particles[a].x - cam.pos.y, particle , 0);
          }
        }
      ...
      Poom.overwritePageAsset(_x, _y, fan, fanFrame + foff);
    }

    // Walkers
    if (walkers[i].active)
    {
      if (Poom.everyFrames(2) && walkers[i].HP > 0 && !walkers[i].hurt)
      {
      ...
      Poom.overwritePageAsset(walkers[i].pos.x - cam.pos.x, walkers[i].pos.y - cam.pos.y, walkerSprite, walkerFrame + (walkers[i].HP <= 0) * 2);
    }

    // Coins
    if (coins[i].active)
    {
      Poom.overwritePageAsset(coins[i].pos.x - cam.pos.x, coins[i].pos.y - cam.pos.y, elements, coinFrame);
    }
  }
}
