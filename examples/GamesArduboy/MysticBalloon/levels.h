...
void drawGrid() {
  ...
  for (byte x = 8; x < 9; --x)
  {
      for (byte y = 5; y < 6; --y)
      {
            //Poom.drawPageAsset(x * spacing - (cam.pos.x >> 2) % spacing, (int)y * spacing - (cam.pos.y >> 2) % spacing, tileSetTwo, 16);
            Poom.drawPageAsset(x * spacing - (cam.pos.x >> 2) % spacing, (int)y * spacing - ((cam.pos.y + 64) >> 2) % spacing, tileSetTwo, 16);
      }
  }
  for ( int x = (cam.pos.x >> 4); x <= (cam.pos.x >> 4) + 8; ++x)
  {
    for ( int y = (cam.pos.y >> 4); y <= (cam.pos.y >> 4) + 4; ++y)
    {
      ...
      //&& y >= 0 && y < LEVEL_HEIGHT)
      {
        ...
        if (tile != 16)
          Poom.overwritePageAsset((x << 4) - cam.pos.x, (y << 4) - cam.pos.y, tileSetTwo, tile);
      }
    }
  }
  //Poom.drawPageAssetInterleavedMasked(levelExit.x - cam.pos.x, levelExit.y - cam.pos.y, sprDoor, walkerFrame);
  //byte frame = 0;
  //if (key.haveKey) frame = walkerFrame + 1;
  //Poom.drawPageAssetInterleavedMasked(levelExit.x - cam.pos.x, levelExit.y - cam.pos.y, sprDoor, (walkerFrame + 1) * (key.haveKey));
  int commonx = levelExit.x - cam.pos.x;
  int commony = levelExit.y - cam.pos.y;
  //Poom.drawPageAsset(commonx, commony, largeMask, 0);
  //Poom.erasePageAsset(commonx, commony, sprDoor, (walkerFrame + 1) * (key.haveKey));
  Poom.overwritePageAsset(commonx, commony, door, (key.haveKey));
  //Serial.println("End of tile drawing");
}

void windNoise()
{
  if (Poom.everyFrames(2)) Poom.audio().tone(320 + random(20), 30);
}
...
void checkCollisions()
{
  ...
  if (collide(keyRect, playerRect) && key.active)
  {
    key.active = false;
    key.haveKey = true;
    Poom.audio().tone(420, 200);
  }

  // Level exit
  HighRect exitRect = {.x = levelExit.x + 4, .y = levelExit.y, .width = 8, .height = 16};
  if (collide(exitRect, playerRect) && Poom.justPressed(PoomButtonUp) && key.haveKey)
  {
  ...
  for (byte i = MAX_PER_TYPE-1; i < MAX_PER_TYPE; --i)
  {
    ...
    if (walkers[i].active)
    {
      ...
      if (collide(playerRect, walkerrect) && walkers[i].HP > 0 && !kid.isImune)
      {
        ...
        // Avoid left-shifting a negative signed value, whose result is not
        // portable across the AVR and ESP32 compilers.
        const int8_t knockback = max(min((kid.pos.x - walkers[i].pos.x - 2), 3), -3);
        kid.speed.x = knockback * (1 << FIXED_POINT);
      }
    }
    // Fans
    if (fans[i].active)
    {
      ...
      if (collide(playerRect, fanrect) && kid.isBalloon)
      {
        ...
        //kid.speed.y = min(kid.speed.y + FAN_POWER, MAX_YSPEED);
        //if (Poom.everyFrames(3)) Poom.audio().tone(330 + random(20), 30);
        windNoise();
...
void drawHUD()
{
  ...
  {
    Poom.drawPageAsset(i * 8, 0, smallMask, 0);
  }
  drawBalloonLives();
  drawNumbers(91, 0, FONT_SMALL, DATA_SCORE);
  //if (coinsCollected < 6 || walkerFrame == 0)
  drawCoinHUD();
  if (key.haveKey) Poom.overwritePageAsset(28, 0, elementsHUD, 13);
}
#endif
