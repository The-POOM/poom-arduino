...
void checkKid()
{
  if (kid.isImune)
  {
    if (Poom.everyFrames(2)) kid.isActive = !kid.isActive;
    kid.imuneTimer++;
  ...
  {
    if (kid.isLanding) Poom.audio().tone(80, 30);
    kid.speed.y = 0;
    kid.speed.x = 0;
    kid.isLanding = false;
    kid.isJumping = false;
    ...
    //if (!Poom.anyPressed(PoomButtonRight) && !Poom.anyPressed(PoomButtonLeft))
    if (!Poom.anyPressed(PoomButtonRight | PoomButtonLeft))
    {
  ...
  if (kid.isSucking) windNoise();//Poom.audio().tone(300 + random(10), 20);
}
...
void drawKid()
{
  if (kid.isActive)
  {
    ...
    if (kid.isBalloon)
    {
      ...
      if (kid.balloons > 1)
      {
        Poom.drawPageAssetInterleavedMasked(commonx + 1, commony - 11, balloon_plus_mask, 0);
        if (kid.balloons > 2) Poom.drawPageAssetInterleavedMasked(commonx + 7, commony - 12, balloon_plus_mask, 0);
      }
      Poom.drawPageAssetInterleavedMasked(commonx + 4, commony - 9, balloon_plus_mask, 0);
    }
    if (!kid.isSucking)
    {
      Poom.drawPageAsset(kidcam.x, kidcam.y, kidSprite, 12 + kid.direction);
      Poom.erasePageAsset(kidcam.x, kidcam.y, kidSprite, kid.frame + 6 * kid.direction + ((kid.isJumping << 2) + 5 * (kid.isLanding || kid.isBalloon)) * !kid.isSucking);
    }

    else
    {
      Poom.drawPageAssetInterleavedMasked(kidcam.x - 2, kidcam.y, kidSpriteSuck_plus_mask, walkerFrame + 2*kid.direction); //kidSpriteSuck
      for (byte i = 0; i < PLAYER_PARTICLES; ++i)
      {
        // Update
        ...
        // Draw
        if (kid.direction)
          Poom.erasePageAsset(kidcam.x - kid.particles[i].x, kidcam.y + 10 + kid.particles[i].y, particle , 0);
        else
          Poom.erasePageAsset(kidcam.x + 15 + kid.particles[i].x, kidcam.y + 10 + kid.particles[i].y, particle , 0);
      }
    }
  }
}
